...
sphereMaterialColor = Vector(red, green, blue)
sphereMaterialSpecularColor = Vector(red,green,blue)
...
for i in range(5):
    sphereRadius = random.uniform(0,.1)
    x = random.uniform(-.5,.5)
    y = random.uniform(-.5,.5)
    z = random.uniform(-.5,.5)
    sphereCenter = Point3D(x,y,z)
    sphere = sphere = Sphere(sphereMaterial, sphereCenter, sphereRadius)
    ...
    for object in spheres:
        if object != originObject:
            ...
            if t >= 0 and t < closestHit:
                closestHit = t
                closestObjectIndex = spheres.index(object)
